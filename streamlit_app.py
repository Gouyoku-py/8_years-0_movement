...
uploaded_file_CS9 = st.file_uploader(label = 'Εισάγετε το αρχείο που προκύπτει από την κίνηση MC.9, εφαρμόζοντας τις οδηγίες της βοήθειας (αμέσως παρακάτω).',
                                     type = 'txt',
...
with st.expander('Βοήθεια για κίνηση MC.9', expanded = False):
    st.markdown(':o:Ανοίξτε την κίνηση **MC.9**, συμπληρώστε τις τιμές *Εγκατάσταση* = 4000, *Αποθηκευτικός Χώρος* = MG01 και *Μήνας* από τον προηγούμενο έως τον τρέχοντα και εκτελέστε την κίνηση **(F8)**.  \n\
...
try:
    ...
    st.markdown('*Όταν εισάγετε ένα έγκυρο αρχείο από την MC.9, στο σημείο αυτό θα εμφανιστεί η επιλογή να κατεβάσετε τους κωδικούς που πρέπει να εισάγετε στην MB51.*')
